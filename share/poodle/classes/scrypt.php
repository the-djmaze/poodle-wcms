...
abstract class Scrypt
{
	 ...
	 */
	public static function kdf($passphrase, $salt, $N, $r, $p, $length, $raw_output = false)
	{
		if ($N == 0 || ($N & ($N - 1)) != 0) {
		...
		if (extension_loaded('Scrypt')) {
			return \scrypt($passphrase, $salt, $N, $r, $p, $length, $raw_output);
		}
		...
		$b = hash_pbkdf2('sha256', $passphrase, $salt, 1, $p * $MFLen, true);
		...
		return hash_pbkdf2('sha256', $passphrase, $salt, 1, $length, $raw_output);
	}
