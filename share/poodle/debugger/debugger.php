...
abstract class Debugger
{
	private static
		...
		$errortypes = array (
			...
			E_USER_DEPRECATED   => 'WCMS Deprecated',
	...
	public static function displayPHPErrors($set=null)
	{
		return !!((\Poodle::$DEBUG & \Poodle::DBG_PHP) || \Poodle\PHP\INI::get('display_errors'));
	}
	...
	{
		return self::$last_error; // error_get_last()
	}
	...
	public static function error($errno, $errmsg, $filename, $linenum, $context=array())
	{
		...
		if ((error_reporting() & $errno) && self::displayPHPErrors()) {
	...
	{
		$K = \Poodle::getKernel();
		return $K && !empty($K->IDENTITY) && $K->IDENTITY->isAdmin();
	}
