	...
	public static function open($filename, $skip_default=true)
	{
		...
		case 'bzip2':
			if (function_exists('bzopen')) {
				if ($bz = bzopen($filename, 'rb')) {
					...
					bzclose($bz);
					if ('ustar' === $id) {
						return new \Poodle\Filesystem\File\Archive\TAR($filename, 'bzip2');
					}
					return new \Poodle\Filesystem\File\Stream\Bzip2($filename);
				}
			} else {
		...
		case 'gzip':
			if ($gz = gzopen($filename, 'rb')) {
				...
				gzclose($gz);
				if ('ustar' === $id) {
					return new \Poodle\Filesystem\File\Archive\TAR($filename, 'gzip');
				}
				return new \Poodle\Filesystem\File\Stream\Gzip($filename);
			}
			return false;

		case 'rar':
			if (function_exists('rar_open')) {
				return new \Poodle\Filesystem\File\Archive\RAR($filename);
			}
		...
		case 'gtar':
		case 'tar':
			return new \Poodle\Filesystem\File\Archive\TAR($filename);

		case 'zip':
			return new \Poodle\Filesystem\File\Archive\ZIP($filename);

		default:
			if ($skip_default) { return false; }
			return new \Poodle\Filesystem\File\Stream\Raw($filename);
		}
	}
...
//			if (!getenv('MAGIC')) { putenv('MAGIC='.__DIR__.'/magic.mime'.(WINDOWS_OS?'.win32':'')); } # /usr/share/misc/magic
		...
		if ('application/zip' === $mime && $file) {
			$zip = new \Poodle\Filesystem\File\Archive\ZIP($file);
	...
	protected static function getMimeFromData($str)
	{
		static $magic;
		if (!$magic) {
			require __DIR__ . '/file/magic.mime.php';
		}
