...
function F()
{
	...
	function HTML5InputElement(o)
	{
		if (!o._ph5fi) {
			...
			if ("password" == o.type && o.hasClass('meter')) {
				var s = K.$C("span", {'class':'passphrase-meter'});
				o.replaceWith(s);
				s.$A(o);
				if (!o.meter) {
					o.meter = K.$C('meter', {min:0, low:35, optimum:85, high:70, max:100, value:0, title:_("Passphrase strength")}).placeAfter(o);
				}
				o.on('input',function(){
					//this.style.backgroundImage = 'linear-gradient(to right, red, orange, green)';
					o.meter.value = this.getPassphraseStrength();
				});
			}
...
F.Number = function(input, force)
{
	...
	if (!input._ph5fi || force) {
		input._ph5fi = "number";
		var s = K.$C("span",{"class":"input-number",input:input,onwheel:function(e){
				var o = this.input;
				if (o && !o.readOnly && !o.disabled) mousewheel(e);
			}
		});
		input.replaceWith(s);
		s.$A(input);
		input
			...
			.on(DCVC,function(){this.trigger("change");});
		s.$A("a",{
			href:"#",
			innerHTML:"▲",
			onclick:noevent,
...
F.Range = function(input, force)
{
	...
	function mouseup()    { target = null; document.off("mouseup",mouseup).off("mousemove",mousemove); }
...
//		o.type = 'text';
		...
		if (!o.readOnly) {
			...
			if (38 == kc || 40 == kc) {
				var m, ml,
					...
					v = getDate(o).format(f),
					s = o.selectionStart,
					p = (0 > n.indexOf(v[s])) ? s : getNPos(v, s);
				while (0 < p && 0 <= n.indexOf(v.charAt(p-1))) --p;
	...
	{
		var c = calendar || createCalendar(), o = e.currentTarget||e;
	...
	{
		cn = cn==no?"current":"";
		var n = o.childNodes[i]||ADivTo(o, cn, selectDateItem).on("selectstart",noevent);
		n.value = no;
	...
	function createCalendar()
	{
		...
		n = c.$ADiv("caption");
		n.$AA("prevMonth", switchMonth/*,"\xA0"*/);
		c.month = n.$AA("select month",showDropDown);
		c.year  = n.$AA("select year", showDropDown);
		n.$AA("nextMonth", switchMonth);
		n.$AA("close", hideCal);

		c.content = c.$A("div", {"class":"content"});
		c.today = c.$ADiv("todaysDate").$AA("todaysDateString", pickTodaysDate, _("Today"));

		c.year.select = c.selects[0] = n = c.$ADiv(sb).hide();
		n.id = "year"+DD;
		n.update = function()
		{
		...
		c.month.select = c.selects[1] = n = c.$ADiv(sb).hide();
		n.id = "month"+DD;
		n.update = function()
		{
...
F.Color = function(input, force)
{
	...
	function RGBColor(r,g,b,a)
	{
		...
		this.l = function(){return intval(rgb1(this).m/2 * 100);};
	...
	if (!input._ph5fi || force) {
		...
		cf=function(){
			if ("radio"==o.type) {
				...
				while (n = fields[i++]) if (n.a && n.name == o.name) {
					n.a[n.checked?"addClass":"removeClass"]("checked");
//					n.a[n.indeterminate?"addClass":"removeClass"]("indeterminate");
				}
			} else {
				o.a[o.checked?"addClass":"removeClass"]("checked");
				o.a[o.indeterminate?"addClass":"removeClass"]("indeterminate");
			}
		};
		o.replaceWith(a);
		a.$A(o)
	...
	if (!o._bmdatalist && o.attr("list")) {
		...
		*/
		...
		if (o._bmdatalist) {
			...
			o.cb.datalist.on('focus',function(){
				this.cb.input.focus();
...
F.Button = function(o)
{
	if (!o._ph5fi) {
		...
		if ("submit"==t || "image"==t) {
			...
			o.on("click", function(){document.location.href = o.attr("formaction");});
		}
	}
};
