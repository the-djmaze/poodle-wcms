...
//		if ($file['tmp_name'] === 'none')
		...
		if (!is_readable($file['tmp_name'])) {
			$tmpfile = POODLE_HOSTS_PATH.'tmp/upload-'.md5(microtime());
		...
		$file['name'] = $this->org_name = \Poodle\Filesystem\File::fixName($file['name']);
		...
		if ('csv' === $ext) {
			...
			$file['type'] = preg_replace('#;.*#','', \Poodle\Filesystem\File::getMime($this->tmpfile));
...
//		case 'dirname':   return dirname($this->moved ?: $this->file['name']);
	...
	public function validateType($allowed_types)
	{
		if (!$this->file['type']) { return false; }
		$match = true;
		if (is_array($allowed_types)) {
			$allowed_types = str_replace('jpg', 'jpeg', implode('|',$allowed_types));
			$match = !!preg_match('#(^|[/\\-])('.$allowed_types.')(\\-|$)#', $this->file['type']);
		} else if ($allowed_types) {
//			$ext = ".{$this->__get('extension')}";
			$ext = \Poodle\Filesystem\File::mimeToExtension($this->file['type']);
			$match = false;
			...
			foreach ($allowed_types as $type) {
				if ('.' === $type[0]) {
					...
					$match |= (0 === strpos($this->file['type'], rtrim($type, '*')));
				}
			}
		}
		if (!$match) {
	...
	public function moveTo($newfile, $ext=null, $overwrite=false)
	{
		...
		# set extension
		$ext = $ext ? ".{$ext}" : \Poodle\Filesystem\File::mimeToExtension($this->file['type']);
	...
	// ArrayAccess
	public function offsetExists($key)  { return array_key_exists($key, $this->file); }
	public function offsetGet($key)     { return isset($this->file[$key]) ? $this->file[$key] : null; }
