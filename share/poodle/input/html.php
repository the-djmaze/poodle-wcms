...
class HTML
{
	...
	private static
		$html_regex = array(
			...
			// LibreOffice
			'#</?sdfield( [^>]*)?>#' => '',
			'#\\s+name="__RefHeading___(Toc[0-9]+)_[0-9]+#' => ' id="$1',
			'#__RefHeading___(Toc[0-9]+)_[0-9]+#' => '$1',
			'#\\s+face="[^"]*"#i' => '',
			// MS Word
			'#<!--\\[(((?!-->).)*)\\]-->#s' => '',
			'#</?(xml|[ovwxp]:\\w+)[^>]*>#si' => '',
...
//			'#<([^>]+)>#e' => 'preg_replace(\'#(\s[a-z-]+)\s*=\s*([^"\\\'\s>]+)#i\', \'\\$1="\\$2"\', str_replace("\\\'","\'","<$1>"))',
			...
			'#<(area|base|br|col|embed|hr|img|input|keygen|link|meta|param|source|track|wbr)([^>]*)/?>#i' => '<$1$2/>',
			'#[\\s\\/]*\\/>#' => '/>',
		),

		$allowed_tags = 'a,abbr,address,article,aside,audio,b,bdo,blockquote,br,canvas,caption,cite,code,col,colgroup,dd,del,details,dfn,div,dl,dt,em,fieldset,figcaption,figure,footer,h1,h2,h3,h4,h5,h6,header,hgroup,hr,i,img,ins,kbd,legend,li,main,mark,menu,meter,nav,ol,p,param,pre,progress,q,samp,section,small,source,span,strong,sub,summary,sup,table,tbody,td,tfoot,th,thead,time,title,tr,ul,var,video',
//		area,base,body,button,datalist,embed,form,head,html,iframe,input,keygen,label,link,map,meta,noscript,object,optgroup,option,output,rp,rt,ruby,script,select,style,textarea
		$self_contained = array('area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr');
...
//		throw new \Exception(xml_error_string(xml_get_error_code($this->parser)));
	...
	protected function xml_node_start($parser, $name, $attribs)
	{
		...
		foreach ($attribs as $k => $v) {
			$k = strtolower($k);
			if ('on' !== substr($k, 0, 2) // JavaScript not allowed
			 && false === strpos($k, ':') // namespaces not allowed (tal:, i18n:, xsl:, etc.)
			 // Accept empty attributes, except: id, title, target
			 && ('' != $v || !in_array($k, array('id','title','target')))) {
				$this->html .= ' '.$k.'="'.htmlspecialchars($v).'"';
			}
		}

//		self::$self_contained = $DTD['EMPTY-TAGS'];
		$this->html .= (in_array($name, self::$self_contained, true) ? '/>' : '>');
	}
	...
	{
		$name = strtolower($name);
		if (in_array($name, self::$self_contained, true)
...
//		return mb_convert_encoding('&#x'.dechex($num).';', 'UTF-8', 'HTML-ENTITIES');
	...
	public static function utf8mb4ToEntities($str)
	{
		return preg_replace_callback(
			...
			function($m){return mb_convert_encoding($m[0],'HTML-ENTITIES');},
			$str);
	}

	private static function reAttributes($m)
	{
		return htmlspecialchars($m[0], ENT_NOQUOTES, null, false);
	}
	...
	private static function reCbTagCSS($m)
	{
		...
		$t = $m[1].$m[4];
		$v = "{$m[2]}:{$m[3]};";
		return strpos($t, 'style=')
			? str_replace('style="', 'style="'.$v, $t)
			: str_replace('>', ' style="'.$v.'">', $t);
	}

	private static function reCbTagLC($m)
	{
