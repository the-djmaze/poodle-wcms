...
const P = Poodle,
self_closing_tags = /<(area|base|br|col|embed|hr|img|input|keygen|link|meta|param|source|track|wbr)([^>]*)>/g,
lc = v => v.toLowerCase(),
body = n => n.$Doc ? n.$Doc().$B() : n,
doc = n => n.$Doc ? n.$Doc() : n.$D(),
win = n => n.$Win ? n.$Win() : n.$W(),
getRange = win =>
{
	...
	return r || (d.createRange ? d.createRange() : d.body.createTextRange());
},
getNode = doc =>
{
...
setTagCSS = (t,v) => 0 < t.indexOf('style=') ? t.replace('style="','style="'+v) : t.replace('>',' style="'+v+'">'),
/*
initFakeElements = o =>
{
	...
	while (els[++i]) {
		if (!els[i].fakeElement) {
			els[i].fakeElement = n = P.$C('span',{'class':'wysiwyg-fake-element',style:{position:'relative'}});
			els[i].replaceWith(n);
			n.$A(els[i]);
...
*/
DOM_DEF_ATTRIBUTES = ['title','class','id'/*,'dir','lang','xml:lang'*/],
ELEMENT_ATTRIBUTES = {
...
//			initFakeElements(a.getBody());
			a.getBody().querySelectorAll('details').forEach(n => {
				if (n.notOpen = !n.open) {
					n.open = true;
				}
			});
	...
	{
		if (current != textarea) {
			current.getBody().querySelectorAll('details').forEach(n => n.open = !n.notOpen);
	...
	this.setConfig = function(cfg)
	{
		...
		var css = (config ? config.css : 'style;poodle_tabs;wysiwyg_base.css');
		write_iframe(htmlarea, node_class, (css?P.CSS_PATH+css:css));
	};
	...
	function create(node)
	{
		...
		if ("textarea" == lc(node.nodeName)) {
			...
			if (defined(window.PoodleResizer)) {
				f.ctrl.resizer = o = new PoodleResizer(f, f, f.ctrl);
				o.onmousedown = o.onmouseup = e => {
					if (current.getNode().$Doc) {
						var d = current.getDocument(), fn = ("mousedown" == e.type)?'on':'off';
						d[fn]("mousemove mouseup", e => f.ctrl.resizer.mouse(e));
					}
				};
				o.onresize = (e, s) => {
					if (e.currentTarget == current.getDocument()) {
						s.x = e.pageX;
						s.y = e.pageY;
					}
				};
...
//			 .off("mousedown", reactivateDesignMode);
			...
			if (P.UA.webkit && !body_observer) {
				body_observer = new MutationObserver(mutations => {
					if (!inserting) {
						mutations.forEach(mutation => {
							mutation.addedNodes.forEach(n => {
								// Workaround Chrome span bug, this creates a new bug as it removes element/styling
								if ("span" == n.lowerName() && !n.children.length && n.style.lineHeight) {
...
//			o.on("mousedown", reactivateDesignMode);
	...
	function update_css_options(initial)
	{
		...
		var i=0, n, v, pop=attribs_window, props=pop.s.$T("input"), t=pop.exampletxt;
	...
	function edit_attributes(node)
	{
		...
		var i, t, k, n, o, v, w=PW, w_id="wysiwyg_attribs_window", pop = attribs_window,
		...
		if (al) {
			...
			for (i=0; i<al; ++i) {
				...
				if ("form" == t && ("method" == k||"enctype" == k)) {
					...
					if ("a" == t && "href" == k) {
						ia.list = "resource_metadata-0-link-canonical-list";
					} else
					if ("maxlength" == k) {
						ia.type = "number";
						ia.min = 0;
...
*/
		...
		P.HTML5Form(pop.f);
		pop.f.on("afterreset",() => update_css_example());
		pop.raw.on("keyup",function(){
		...
		if (n) {
			...
			while (n && n != doc && 'body' != tag) {
				if (!n.hasClass('wysiwyg-fake-element')) {
					...
					while (btn = btns[i++]) {
						if (P.strContains(btn.className, cn)) {
							btn.addClass("active");
						}
					}
					if ("a" == tag) {
...
PW.elementMap = new Array(
	...
	// IE
	[/<([^>]+)>/gm, (d,tag) => '<'+tag.replace(/(\s[a-z-]+)\s*=\s*([^"'\s>]+)/gmi, '$1="$2"')+'>'], // fix attributes
	[/"[\t ]*([^"=\r\n><]+)[\t ]*"/gm, '"$1"'],
	[/(<[^>]+)\s+(background-color)="(#[A-F0-9]+)"([^>]*>)/gim, (d,tag,prop,value,tag_e) => setTagCSS(tag+tag_e, prop+":"+value+";")],
	// Attributes to css
	[/(<[^>]+)\s+color="(#[A-F0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "color:"+value+";")],
	[/(<[^>]+)\s+bgcolor="(#[A-F0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "background-color:"+value+";")],
	[/(<[^>]+)\s+valign="(top|middle|bottom|baseline)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "vertical-align:"+value+";")],
	[/(<[^>]+)\s+width="([0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "width:"+value+"px;")],
	[/(<[^>]+)\s+height="([0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "height:"+value+"px;")],
	[/(<[^>]+)\s+border="([0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "border-width:"+value+"px;")],
	[/(<[^>]+)\s+bordercolor="(#[A-F0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "border-color:"+value+";")],
	[/(<[^>]+)\s+cellspacing="([0-9]+)"([^>]*>)/gim, (d,tag,value,tag_e) => setTagCSS(tag+tag_e, "border-spacing:"+value+"px;")],
	...
	[/\r/g, ''],
	// Init tabs - new tab
	[/<li class="new">[\s\S]*<\/li>/gi,''],
	// strip FakeElements
	[/<span[^>]+class="wysiwyg-fake-element-overlay"[^>]*>[^<]*<\/span>/g,''],
	[/<span[^>]+class="wysiwyg-fake-element"[^>]*>([\s\S]*)?<\/span>/g,'$1'],
	// Google docs
