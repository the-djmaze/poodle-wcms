...
abstract class LOG
{
	...
	protected static function insert($level, $type, $msg, $log_headers=false)
	{
		...
		{
			if ($level > 3 && isset($K->CFG->poodle->log_levels) && strlen($K->CFG->poodle->log_levels)
			 && !in_array($level, explode(',',$K->CFG->poodle->log_levels))) {
				return;
			}
	...
	public static function onIdentityDelete(\Poodle\Events\Event $event)
	{
		if ($event->target instanceof \Dragonfly\Identity) {
			\Dragonfly::getKernel()->SQL->TBL->log->delete(array(
