...
class FixBody
{
	...
	private static
		$EMPTY_TAGS = array('area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr'),
...
//				trigger_error("Attribute {$k} not allowed in {$name}");
				...
				if (('href'===$k || 'src'===$k || 'background'===$k) && !strpos($v, ':')) {
					$v = \Poodle\URI::abs(\Poodle\URI::resolve($v));
				}
...
//		'#<(b|strong)[^>]*>(.*?)</\\1>#ie' => 'strtoupper("$2")', // <b> | <strong>
	...
	protected function text_node_start($parser, $name, $attribs)
	{
		...
		else if (preg_match('/^(br|div|li|tr)$/',$name))
		{
			$this->data .= "\n";
			if ('li' === $name) {
				if ($this->numbers['ol']) {
