...
*/
	...
	function __construct($cfg='', $html=true)
	{
		...
		$this->from->append("noreply@{$_SERVER['HTTP_HOST']}");
		$this->tpl_type = $html ? 'html' : 'txt';
		$this->cfg = $cfg;
		if ($K->CFG) {
			$this->setFrom($K->CFG->mail->from ?: "noreply@{$_SERVER['HTTP_HOST']}", $K->CFG->site->name);
...
//			if (count($this->from) > 1) return $this->sender;
	...
	function __set($k, $v)
	{
		...
		case 'body':       $this->$k = $v; $this->body_plain = ''; break;
		case 'body_plain': $this->$k = $v; break;
	...
	public function addNotifyTo($address, $name='')
	{
		$this->notify_to->append($address, $name);
	}
	...
	public function prepare(&$header, &$body, $flags=0)
	{
		...
		if (5 < $count_to + $count_cc && !$this->allow_multiple_to) {
			throw new \Exception(self::l10n('There are too much To/Cc recipients'), E_USER_ERROR);
		}
		if (!$count_to && !$count_cc) {
			if (!$count_bcc) {
				throw new \Exception(self::l10n('Must provide at least one recipient email address'), E_USER_ERROR);
			}
			$this->recipients['To']->append($this->from[0]->address, 'Undisclosed Recipients');
		}
		...
		if (count($this->reply_to)) {
			...
			if (preg_match('/@(gmail|hotmail|live)\\./', $this->reply_to[0]->address, $m)) {
				if (!strpos($this->from[0], $m[1])) {
					trigger_error("FREEMAIL_FORGED_REPLYTO Freemail in Reply-To, but not From ({$this->reply_to[0]->address} / {$this->from[0]->address})");
				}
				if ($this->reply_to[0]->address !== $this->from[0]) {
					trigger_error("FREEMAIL_REPLYTO Reply-To/From contain different freemails ({$this->reply_to[0]->address} / {$this->from[0]->address})");
				}
			}
		}
		...
		if ($count_bcc && ($flags & self::HEADER_ADD_BCC)) {
...
//		$header[] = "X-MimeOLE: Produced By Poodle WCMS";
		$header[] = "X-Mailer: Poodle/" . \Poodle::VERSION;
		...
		if (count($this->notify_to)) {
			$header[] = 'Disposition-Notification-To: '.$this->notify_to->asEncodedString();
		}
		# Add custom headers
