...
class Discover
{
	...
	public static function ID(string $identifier, \Poodle\HTTP\Request $fetcher) : array
	{
		// 7.2 step 1:
	...
	// 7.2 step 3 & 4 + 7.3 step 2 & 3
	protected static function Yadis(string $url, \Poodle\HTTP\Request $fetcher) : ?array
	{
		// 7.2 step 3:
	...
	public static function filter_MatchesAnyType(\Poodle\Yadis\XRDS_Service $service)
	{
	 ...
	 */
	protected static function getOPOrClaimedServices(iterable $services) : array
	{
		$op_services = self::sortByType($services, array(\Poodle\OpenID::TYPE_V2_0_OP));
		return $op_services ?: self::sortByType($services, \Poodle\OpenID::getTypeURIs());
	}

	protected static function sortByType(iterable $service_list, iterable $preferred_types) : array
	{
	...
	protected static function fetch_href($html) : ?string
	{
		return preg_match('#^.+href=["\'](.+)?["\'].+$#Dsi', $html, $m) ? \Poodle\RFC_3986::normalize_url(htmlspecialchars_decode($m[1])) : null;
	}

	protected static function makeServiceEndpoints(string $uri, iterable $yadis_services) : array
	{
		$s = array();
		if ($yadis_services) {
