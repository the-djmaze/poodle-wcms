...
class PgSQL implements \Poodle\SQL\Interfaces\Adapter
{
	...
	function __construct($config)
	{
		if (!function_exists('pg_connect')) {
			throw new \Poodle\SQL\Error('PostgreSQL', \Poodle\SQL\Error::NO_EXTENSION);
		}
	...
	public function connect()
	{
		...
		if (!($this->connection = pg_connect($connect_string))) {
			throw new \Poodle\SQL\Error(pg_last_error(), \Poodle\SQL\Error::NO_CONNECTION);
		}
...
//		$q = 'SELECT table_name FROM information_schema.tables WHERE table_type = \'BASE TABLE\' AND table_schema NOT IN (\'pg_catalog\', \'information_schema\')';
	...
	public function quoteString($str)     { return "'".pg_escape_string($str)."'"; }
...
*/
	...
	public function upsert($table, array $insert_data, array $update_data, array $update_where)
	{
		...
		try {
			...
			return $this->query("INSERT INTO {$table} (".implode(', ', array_keys($insert_data)).') VALUES ('.implode(', ', $insert_data).')');
		} catch (\Throwable $e) {
			foreach ($update_data as $field => &$value) {
				$value = $field.'='.$value;
			}
			if (!$update_where) {
				$update_where = array_diff_key($insert_data, $update_data);
			}
			foreach ($update_where as $field => &$value) {
				$value = $field.'='.$value;
			}
			return $this->query("UPDATE {$table} SET ".implode(', ', $update_data)." WHERE ".implode(' AND ', $update_where));
		}
	}
}
