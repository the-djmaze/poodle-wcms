...
abstract class Importer extends \XMLReader
{
	...
	protected function importData($file, $old_version, $file_is_data = false)
	{
		...
		try {
				...
				case self::END_ELEMENT:
					if ($table) {
						...
						if ($this->checkVersion($query, $old_version) && $this->validPlatform($query)) {
							$this->doQuery(\preg_replace('#{([a-z0-9_]+)}#s', $this->tbl_prefix.'$1', $data));
						} else {
						...
						if ($data && $this->validPlatform($view)) {
							$this->doQuery("CREATE OR REPLACE VIEW {$this->tbl_prefix}{$view['name']} AS "
								.\preg_replace('#{([a-z0-9_]+)}#s', $this->tbl_prefix.'$1', $data));
						} else {
			...
			if (0 !== $this->depth) {
				$this->errors[] = array('message' => 'Unknown XML parse error', 'query' => '');
				$this->SQL->rollback();
				return false;
			}
			$this->close();
	...
	protected function loadData($xml, $file, $file_is_data = false, $external_entities = true)
	{
		...
		libxml_disable_entity_loader(!$external_entities);
	...
	protected function setXMLErrors()
	{
		foreach (libxml_get_errors() as $error) {
			$this->errors[] = array('message' => trim($error->message)." on line {$error->line}");
		}
		libxml_clear_errors();
	}
...
/*
				if ('http://www.w3.org/2001/XMLSchema-instance' == $this->namespaceURI) {
					$attributes[$this->localName] = $this->value;
				} else {
...
*/
	...
	protected function doQuery(string $query, int $inc = 1)
	{
		\set_time_limit(\Poodle\PHP\ini::get('max_execution_time'));
//		$this->triggerBeforeQuery($query, $inc);
		$this->SQL->query($query);
		$this->triggerAfterQuery($query, $inc);
	}

	protected $aq_event;
	protected function triggerAfterQuery(string $query, int $inc = 1)
	{
