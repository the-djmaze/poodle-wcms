...
/*	Poodle WCMS, Copyright (c) MH X Solutions since 2016. All rights reserved.
	...
	Tar file creation class.
*/

namespace Poodle\Stream;

class TAR implements Interfaces\Archive
{
	const
		TYPE_FILE = '0',
		TYPE_LINK = '2',
	...
	public function addDir(string $dirname, \SplFileInfo $fileinfo = null)
	{
		$this->writeEntryHeader(
			rtrim(strtr($dirname, '\\', '/'), '/') . '/',
			static::TYPE_DIR,
	...
	protected function start()
	{
		...
		$this->started = true;
		if (!$this->started && false !== $this->gzip) {
			// Write gzip header, see http://www.zlib.org/rfc-gzip.html#member-format
			if (!fwrite($this->out, "\x1F\x8B\x08\x00".pack('V', time())."\0\x03", 10)) {
...
//				'window' => 32768,
	...
	protected function writeUStarEntryHeader($name, $type, $size, $uid = 0, $gid = 0, $perm = 0, $mtime = 0, $link = '', $prefix  = '')
	{
		$data = pack('a100a8a8a8a12A12',
			...
			sprintf("%011u", decoct($mtime ?: time())));
		$this->write($data);
		$checksum = 0;
		$i = 148;
		while ($i--) {
...
class TarTempResource extends \SplTempFileObject
{
	...
	public function getGroup() { return 0; }
	public function getPerms() { return 420; }
}
