...
class ZIP implements Interfaces\Archive
{
	private
		$encryption,  // TODO
		$compression,
	...
	public function pushHttpHeaders($name)
	{
		\Poodle::ob_clean();
		header('Cache-Control: no-store, no-cache, must-revalidate');
		header('Pragma: no-cache');
		header('Content-Transfer-Encoding: binary');
		\Poodle\HTTP\Headers::setContentDisposition('attachment', array('filename' => "{$name}.zip"));
		\Poodle\HTTP\Headers::setContentType('application/zip', array('name' => "{$name}.zip"));
	}
...
class ZipResource /* SplFileObject */
{
	...
	public function eof() { return feof($this->source); }
...
class ZipEntry
{
	...
	public function updateCrc32(string $data)
	{
		if (!$this->hctx) {
	...
	// Data descriptor
	public function getDataDescriptor(bool $signature = false)
	{
	...
	public function getDirEntry(int $offset)
	{
		// now add to central directory record
